...
//		offset : 건너 뛸 갯수 / fetch first(next) : 가져올 갯수
		String sql = "select * from board1 order by ref desc, re_step " + "offset ? rows fetch first ? rows only";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, startRow);			//	┐
			pstmt.setInt(2, ROW_PER_PAGE);		//	├	설정 후 실행 순서 주의 !!
	...
	public int insert(Board board) {
		...
		try { // 가장 큰번호 + 1, 입력할 번호 num 생성
			...
			}else board.setRef(number);	//답변글이 아닐 때는 num과 ref가 같음
